...
class KeogramGenerator():
  ...
  def make_keogram(self, path, prescale, prerotate):
    ...
    try:
      for i, f in enumerate(files):
        ...
        input_img = cv2.imread(file_path, cv2.IMREAD_REDUCED_COLOR_8)
        
        resized_img = cv2.resize(input_img, (prescale, prescale),interpolation=cv2.INTER_NEAREST)
        height, width = resized_img.shape[:2]
        centerX, centerY = (width // 2, height // 2)

        M = cv2.getRotationMatrix2D((centerX, centerY), -prerotate, 1.0)
        rotated_image = cv2.warpAffine(resized_img, M, (width, height))
        ...
        col_end = int(width / 2)
        slice = rotated_image[0:height, col_start:col_end] # r, c
        if on_first_image:
          on_first_image = False
          keogram = np.zeros((prescale, keo_width, 3), np.uint8)
          cv2.resizeWindow("keo", keo_width, prescale)
        keogram[0:prescale, curr_column:curr_column + 1] = slice
        curr_column += 1
        if not i%10:
  ...
  def make_contact_sheet(self, path, prescale, prerotate, rows=8, columns=8, image_size=300):
    image_padding_pixels = 8
    contact_sheet_width = (image_size + image_padding_pixels) * columns
    contact_sheet_height = (image_size + image_padding_pixels) * rows
    ...
    files_for_contact_sheet = files[offset_start: offset_start + (step * file_sample_count): step]
    ...
    try:
      for i, f in enumerate(files_for_contact_sheet):
        ...
        if on_first_image:
          ...
          cv2.resizeWindow("contact", contact_sheet_width, contact_sheet_height)
        col_start = (i % columns) * (image_size + image_padding_pixels)
        col_end = col_start + image_size
        row_start = (i // columns) * (image_size + image_padding_pixels)
        row_end = row_start + image_size
        try:
          contact_sheet[row_start:row_end, col_start:col_end] = img
        except:
          print(f"ERROR: could not place image on contact_sheet: {row_start}:{row_end}, {col_start}:{col_end}")
          print(f"exception details: {sys.exc_info()[0]}: {sys.exc_info()[1]}")
        ...
        cv2.imshow("contact", contact_sheet)
        cv2.waitKey(1)

      
      output_path = os.path.join(path, "contact_sheet3.png")
  ...
  def process_cli(self):
    ...
    parser.add_argument('--metadata-M100', default=False, action='store_true', required=False, help='create metadata file from Canon M100/M200')
    parser.add_argument('--metadata-ZVE10', default=False, action='store_true', required=False, help='create metadata file from sony ZVE10')
    parser.add_argument('--transform-metadata', default=False, action='store_true', required=False, help='create transform-metadata file based on GUI selection of orientation and mag declination')
    ...
    for arg, value in self.args.items():
      print(arg, value)
    self.generate_metadata_file_flag_M100 = self.args["metadata_M100"]
    self.generate_metadata_file_flag_ZVE10 = self.args["metadata_ZVE10"]
    self.generate_transform_metadata_file_flag = self.args["transform_metadata"]
    self.generate_contact_sheet_flag = self.args["contactsheet"]
    self.generate_keogram_flag = self.args["keogram"]
